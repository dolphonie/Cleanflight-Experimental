...
_Bool shouldAutotune(void)
{
    return (armingFlags & (ARMED)) && (flightModeFlags & (AUTOTUNE_MODE));
}
        ...
        uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    for (axis = 0; axis < 3; axis++) {
        if (((flightModeFlags & (ANGLE_MODE)) || (flightModeFlags & (HORIZON_MODE))) && (axis == FD_ROLL || axis == FD_PITCH)) {
            ...
            if (shouldAutotune()) {
                errorAngle = (autotune(rcAliasToAngleIndexMap[axis], &inclination, (errorAngle / 10.0f)) * 10);
            }
...
rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    for (axis = 0; axis < 2; axis++) {
        ...
        if ((flightModeFlags & (ANGLE_MODE)) || (flightModeFlags & (HORIZON_MODE))) {
            ...
            if (shouldAutotune()) {
                error = (autotune(rcAliasToAngleIndexMap[axis], &inclination, (error / 10.0f)) * 10);
            }
