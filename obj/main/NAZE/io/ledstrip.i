...
void applyLedThrustRingLayer(void)
{
    ...
    for (ledIndex = 0; ledIndex < ledCount; ledIndex++) {
        ...
        _Bool applyColor = 0;
        if ((armingFlags & (ARMED))) {
            if ((ledRingIndex + rotationPhase) % 6 < 2) {
                applyColor = 1;
            }
        } else {
            if (nextLedOn == 0) {
...
void updateLedStrip(void)
{
 ...
 if (!(ledStripInitialised && isWS2811LedStripReady())) {
    ...
    _Bool indicatorFlashNow = indicatorFlashNow = (int32_t)(now - nextIndicatorFlashAt) >= 0L;
    _Bool warningFlashNow = warningFlashNow = (int32_t)(now - nextWarningFlashAt) >= 0L;
