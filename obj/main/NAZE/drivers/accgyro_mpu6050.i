...
static const mpu6050Config_t *mpu6050Config = ((void *)0);
...
# 210 "./src/main/drivers/accgyro_mpu6050.c"
}
...
void mpu6050GpioInit(void) {
    ...
    static _Bool mpu6050GpioInitDone = 0;
    ...
    mpu6050GpioInitDone = 1;
}

static _Bool mpu6050Detect(void)
{
    _Bool ack;
    uint8_t sig;
...
_Bool mpu6050AccDetect(const mpu6050Config_t *configToUse, acc_t *acc)
{
    ...
    i2cRead(0x68, 0x06, 6, readBuffer);
    ...
    acc->revisionCode = (mpuAccelTrim == MPU_6050_HALF_RESOLUTION ? 'o' : 'n');

    return 1;
}

_Bool mpu6050GyroDetect(const mpu6050Config_t *configToUse, gyro_t *gyro, uint16_t lpf)
{
    mpu6050Config = configToUse;
...
static void mpu6050GyroInit(void)
{
    ...
    delay(100);
    i2cWrite(0x68, 0x19, 0x00);
    i2cWrite(0x68, 0x6B, 0x03);
    delay(15);
    i2cWrite(0x68, 0x1A, mpuLowPassFilter);
    i2cWrite(0x68, 0x1B, INV_FSR_2000DPS << 3);



    i2cWrite(0x68, 0x1C, INV_FSR_8G << 3);

    i2cWrite(0x68, 0x37,
            0 << 7 | 0 << 6 | 0 << 5 | 0 << 4 | 0 << 3 | 0 << 2 | 1 << 1 | 0 << 0);


    i2cWrite(0x68, 0x38, (1 << 0));
