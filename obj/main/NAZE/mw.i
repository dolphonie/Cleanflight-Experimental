...
# 73 "./src/main/mw.c" 2
# 1 "./src/main/flight/autotune.h" 1
# 18 "./src/main/flight/autotune.h"
...
_Bool hasAutotunePhaseCompleted(void);
...
{
    static _Bool landedAfterAutoTuning = 0;
    static _Bool autoTuneWasUsed = 0;

    if (((1 << BOXAUTOTUNE) & rcModeActivationMask)) {
        if (!(flightModeFlags & (AUTOTUNE_MODE))) {
            if ((armingFlags & (ARMED))) {
                if (isAutotuneIdle() || landedAfterAutoTuning) {
                    autotuneReset();
                    landedAfterAutoTuning = 0;
                }
                autotuneBeginNextPhase(&currentProfile->pidProfile);
                enableFlightMode(AUTOTUNE_MODE);
                autoTuneWasUsed = 1;
            } else {
    ...
    if ((flightModeFlags & (AUTOTUNE_MODE))) {
        autotuneEndPhase();
        disableFlightMode(AUTOTUNE_MODE);
    }

    if (!(armingFlags & (ARMED)) && autoTuneWasUsed) {
        landedAfterAutoTuning = 1;
    }
}
