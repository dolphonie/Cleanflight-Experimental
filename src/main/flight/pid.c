...
pidControllerFuncPtr pid_controller = pidLuxFloat; // which pid controller are we using, defaultMultiWii
        ...
        uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        errorGyroIf[axis] = errorGyroIf[axis] + RateError * dT * pidProfile->I_f[axis] * PIDweight[axis] / 10;
